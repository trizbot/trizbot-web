...
export class FeedPostDialogComponent implements OnInit, OnDestroy {
  ...
  /**
   * Defense-in-depth: this dialog is only ever opened by the parent
   * FeedComponent's openCreateDialog()/openEditDialog(), which already
   * gate on isSuperAdmin. This second, independent check re-verifies
   * against the server so the dialog can never be used to create/update
   * a post even if it were somehow triggered without that guard (e.g. a
   * future caller that forgets the check). Not a substitute for backend
   * authorization — the API must reject non-super-admin writes too.
   */
  private destroy$ = new Subject<void>();

  /**
   * Read-only, auto-generated by the backend on create. Shown in edit mode
   * once the record has one; never editable and never sent back on save.
   */
